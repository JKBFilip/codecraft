...
class UserAccount:
    ...
    def add_test_result(self, module_index, score, perfect_score=False, time_taken=None):
        ...
        test_data = {
            ...
            "is_final_exam": (module_index == 0)  # Dodajemy flagę dla egzaminu końcowego
        }
    ...
    def is_module_fully_completed(self, lesson_index):
        ...
        # Debug: Wyświetl stan modułu
        print(f"\n[DEBUG] Sprawdzanie modułu {lesson_index}:")
        print(f"Zadania ukończone: {self.module_scores.get(str(lesson_index), 0)}/{self.TASKS_PER_MODULE}")
        ...
        # Debug: Wyświetl historię testów
        print("Historia testów:")
        for t in self.test_history:
            if t.get('module') == lesson_index:
                print(f"Test moduł {t.get('module')}, wynik: {t.get('score')}%")

        # Sprawdzamy czy test został zaliczony (wymagane minimum 80%)
        test_passed = any(
        ...
        print(
            f"Moduł {lesson_index} - Zadania: {'TAK' if tasks_done else 'NIE'}, Test: {'TAK' if test_passed else 'NIE'}")
    ...
    def are_all_modules_completed(self):
        """Sprawdza czy wszystkie moduły są w pełni ukończone (zadania + testy)"""
        for module in range(1, 6):  # Moduły 1-5
            if not self.is_module_fully_completed(module):
                return False
        return True

    def has_passed_final_exam(self):
        """Sprawdza czy użytkownik zdał już egzamin końcowy"""
        return any(
            t.get('is_final_exam', False) and float(t.get('score', 0)) >= 80.0
            for t in self.test_history)

    def get_final_exam_result(self):
        """Pobiera wynik egzaminu końcowego jeśli istnieje"""
        for t in self.test_history:
