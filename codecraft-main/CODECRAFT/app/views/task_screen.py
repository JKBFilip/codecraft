...
class TaskScreen(QWidget):
    ...
    def load_tasks(self):
        ...
        for i, task in enumerate(self.filtered_tasks):
            print(f"[DEBUG] Task {i}: ID = {task.get_id()}, index = {task.task_index}, type = {task.type}")
        ...
        print("=== DEBUG: ID zadań ===")
        for i, task in enumerate(self.filtered_tasks):
            print(f"Zadanie {i + 1}: ID = {task.get_id()} / task_index = {task.task_index}")
    ...
    def show_hint(self):
        """Pokazuje podpowiedź do bieżącego zadania"""
        if not self.current_task:
            return

        hint_text = self._prepare_hint()

        # Pokaż okno dialogowe z podpowiedzią
        dialog = HintDialog(hint_text, self)
        dialog.exec()

    def _prepare_hint(self):
        """Przygotowuje tekst podpowiedzi z rozwiązaniem"""
        if not self.current_task:
            return "Brak dostępnych podpowiedzi"

        # Dla zadań wielokrotnego wyboru
    ...
    def check_answer(self):
        ...
        if is_correct:
            print(f"[DEBUG] Zaliczone poprawnie: {self.current_task.get_id()}")
            ...
            self._mark_task_completed()
            print(f"[DEBUG] Teraz ukończone zadania: {progress.completed_tasks}")
        # Update UI feedback
        self._update_ui_feedback(is_correct)

    def save_task(self):
        """Zapisuje aktualną odpowiedź użytkownika bez oceniania"""
        if not self.current_task:
            return
        ...
        # Zapisz odpowiedź użytkownika (nawet jeśli niepoprawna)
        self.main_window.user_progress.save_task_answer(task_id, user_answer)
        print(f"[DEBUG] Odpowiedź do zadania {task_id} została zapisana.")
