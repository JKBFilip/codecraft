...
## ===================================================================
# Zakładam, że ta klasa jest już poprawiona i działa
class CertificateGenerator:
    ...
    def generate(username, score, date_str, output_path="certificates"):
        ...
        try:
            ...
            painter.setPen(border_color)
            main_text = (
            f"Niniejszym potwierdza się, że", f"<b>{username}</b>", f"pomyślnie ukończył(a) egzamin końcowy",
...
class FinalExamScreen(QWidget):
    ...
    def _create_start_page(self):
        ...
        layout.setSpacing(20)  # Zwiększmy odstępy
        ...
        self.start_button.setObjectName("primaryButton")  # Użyjemy stylu
        self.start_button.clicked.connect(self._start_exam)

        # --- NOWY PRZYCISK POWROTU ---
        back_button = QPushButton("🔙 Powrót do menu")
        back_button.setObjectName("secondaryButton")  # Użyjemy stylu dodatkowego
        back_button.clicked.connect(self.main_window.show_menu)  # Podłączamy do funkcji w MainWindow
        # -----------------------------

        layout.addWidget(self.start_info_label)
        layout.addWidget(rules_label)
        layout.addSpacing(30)  # Dodatkowy odstęp przed przyciskami
    ...
    def _create_exam_page(self):
        ...
        # Ramka na pytanie i odpowiedź dla lepszego wyglądu
        question_frame = QFrame();
        question_frame.setObjectName("formFrame")  # Użyjemy stylu ramki z logowania
        ...
        # Kontener na różne typy odpowiedzi
        self.answer_stack = QStackedWidget()
        self.code_input = QTextEdit()  # Użyje domyślnego stylu QLineEdit z style.qss
        self.option_select = QComboBox()  # Domyślny styl też powinien być ok
        self.reorder_list = ReorderList()  # Ten widget może wymagać własnego stylu
        ...
        layout.addWidget(question_frame)  # Dodajemy ramkę do głównego layoutu

        self.submit_button = QPushButton("Zatwierdź odpowiedź");
        self.submit_button.setObjectName("primaryButton")  # Użyjemy stylu
    ...
    def _create_results_page(self):
        ...
        self.summary_text.setObjectName("summaryText")  # Użyjemy stylu podsumowania
        self.download_cert_button = QPushButton("📄 Pobierz certyfikat");
        self.download_cert_button.setObjectName("primaryButton")  # Użyjemy stylu
        self.download_cert_button.clicked.connect(self.download_certificate)
        back_button = QPushButton("🔙 Powrót do menu");
        back_button.setObjectName("secondaryButton")  # Użyjemy stylu
    ...
    # --- GŁÓWNA METODA WEJŚCIOWA (BRAMKARZ) ---
    ...
    def load_task(self):
        """✅ **POPRAWIONO:** Czyści pola przed załadowaniem nowego zadania."""
        ...
        if task_type in ("code_input", "code_output"):
            ...
            self.option_select.addItems(self.current_task.options)  # Ładujemy opcje
    ...
    def check_answer(self):
        ...
        try:  # Dodajemy try-except dla bezpieczeństwa
            if task.type == "code_input":
                ...
                # Proste porównanie stringów, ignorując białe znaki na końcach
                is_correct = (user_answer == correct_answer.strip())
            elif task.type == "multiple_choice":
                user_answer = self.option_select.currentText()
                # Sprawdzamy, czy odpowiedź użytkownika ZACZYNA SIĘ od poprawnej (np. "A. Odpowiedź")
                ...
                # Porównanie kodu linijka po linijce, ignorując białe znaki
            ...
            is_correct = False  # W razie błędu uznajemy za niepoprawną
        ...
        if is_correct:
            ...
            print(f"DEBUG: Pytanie {self.current_index + 1} - POPRAWNA odpowiedź!")
        else:
            print(
                f"DEBUG: Pytanie {self.current_index + 1} - BŁĘDNA odpowiedź. User: '{user_answer}', Correct: '{correct_answer}'")
    ...
    def end_exam(self, timeout=False):
        ...
        # ✅ **KLUCZOWA POPRAWKA:** Oblicz i ustaw `self.exam_passed` TUTAJ
        self.exam_passed = (self.score_percentage >= 80) and not timeout

        # Zapisz wynik (ta część była poprawna)
        self.main_window.user_progress.add_test_result(
            module_index=0,
            score=self.score_percentage,
            # Poprawka: perfect_score też obliczamy tutaj
            perfect_score=(self.correct_answers == total_questions and total_questions > 0),
            time_taken=elapsed_time
        )
        self.main_window.user_progress.check_achievements()  # Sprawdź osiągnięcia po zapisie

        # Wywołaj pokazanie wyników (teraz _show_results_page użyje już ustawionego self.exam_passed)
        self._show_results_page(timeout=timeout)

    def _show_results_page(self, previous_pass=False, timeout=False):
        if previous_pass:
            # Logika dla wcześniej zdanego egzaminu (bez zmian)
            ...
            if result:
                ...
                self.exam_passed = True  # Ustaw flagę, by certyfikat działał
            else:  # Błąd wczytywania
            ...
            # Logika dla NOWO zakończonego egzaminu
            # Używamy self.exam_passed ustawionego w end_exam
            required = 16
            self.summary_text.setHtml(
                generate_summary_html(self.user_answers, self.correct_answers, required_to_pass=required))

            # Ustawiamy tytuł na podstawie self.exam_passed i timeout
            if timeout:
            ...
            # Pokazujemy przycisk certyfikatu tylko jeśli zdany
            self.download_cert_button.setVisible(self.exam_passed)

            # Przełączamy widok na stronę wyników
        self.stack.setCurrentWidget(self.results_page)

    def download_certificate(self):
        if not getattr(self, 'exam_passed', False): QMessageBox.warning(self, "Błąd",
                                                                        "Egzamin nie został zaliczony."); return
    ...
    def _prepare_exam_tasks(self):
        try:
            ...
            return exam_tasks[:20]  # Zwróć maksymalnie 20 zadań
        except Exception as e:
