...
from PySide6.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QTextEdit, QSizePolicy,QDialog, QTextBrowser,QMessageBox
from PySide6.QtCore import Qt
import webbrowser

# W pliku lesson_screen.py, PRZED klasą LessonScreen

# Funkcja wczytująca/definiująca Zadania Programisty
# W przyszłości wczytaj to z pliku JSON
class CodeViewerDialog(QDialog):
    """Proste okno dialogowe do wyświetlania kodu."""
    def __init__(self, code_string, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Rozwiązanie Zadania")
        self.setMinimumSize(600, 400) # Dajmy mu trochę miejsca

        layout = QVBoxLayout(self)

        # Używamy QTextEdit - idealne do pokazywania kodu
        code_edit = QTextEdit()
        code_edit.setReadOnly(True)
        code_edit.setPlainText(code_string.strip()) # Wstawiamy kod jako zwykły tekst
        code_edit.setFont(QFont("Consolas", 11)) # Ustawiamy czcionkę kodu
...
PROGRAMMER_TASKS = {
    ...
    # Można też zaokrąglić wynik dla lepszej czytelności, np. do 2 miejsc po przecinku:
    # print(f"{kilometry_liczba} kilometrów to {mile:.2f} mil.")
    """
    },
    2: {
        "instruction": """
...
"""
    ...
    # Dodaj tutaj zadania dla modułów 3, 4, 5...
    3: {"instruction": "<h2>Zadanie Programisty: Tabliczka Mnożenia ✖️</h2>Napisz program używający zagnieżdżonych pętli `for`, który wypisze tabliczkę mnożenia od 1 do 5 (każdy wynik w osobnym `print`).", "solution": "for i in range(1, 6):\n    for j in range(1, 6):\n        print(f'{i} * {j} = {i*j}')\n    print('-'*10) # Separator"},
    4: {
        "instruction": """
...
"""
    ...
    5: {"instruction": "<h2>Zadanie Programisty: Statystyki Listy 📊</h2>Napisz funkcję `statystyki(lista)`, która przyjmuje listę liczb i **zwraca** słownik zawierający trzy klucze: 'min' (najmniejsza wartość), 'max' (największa wartość) i 'srednia' (średnia arytmetyczna). Przetestuj funkcję na liście `[3, 1, 4, 1, 5, 9, 2]`.", "solution": "def statystyki(lista):\n    if not lista:\n        return {'min': None, 'max': None, 'srednia': None}\n    return {\n        'min': min(lista),\n        'max': max(lista),\n        'srednia': sum(lista) / len(lista)\n    }\n\ndane = [3, 1, 4, 1, 5, 9, 2]\nprint(statystyki(dane))"}
}

def load_programmer_task_data(lesson_index):
    """Zwraca dane zadania dla danego modułu."""
    return PROGRAMMER_TASKS.get(lesson_index, {"instruction": "Brak zadania dla tego modułu.", "solution": ""})


# W pliku lesson_screen.py, PRZED klasą LessonScreen

class ProgrammerTaskDialog(QDialog):
    """Okno dialogowe do wyświetlania Zadania Programisty."""
    ...
    def __init__(self, instruction, solution, parent=None):
        ...
        # Używamy QTextBrowser zamiast QTextEdit, aby linki były klikalne
        self.instruction_browser = QTextBrowser()
        self.instruction_browser.setOpenExternalLinks(True)  # Otwiera linki w przeglądarce
        ...
        button_layout.addStretch()  # Wypycha przyciski na prawo

        self.solution_button = QPushButton("💡 Pokaż rozwiązanie")
        self.solution_button.setObjectName("secondaryButton")  # Używamy stylu przycisku dodatkowego
        self.solution_button.clicked.connect(self.show_solution)

        close_button = QPushButton("Zamknij")
        close_button.clicked.connect(self.accept)  # Standardowy sposób zamykania QDialog
    ...
    def show_solution(self):
        """Wyświetla rozwiązanie w osobnym okienku."""
        # Używamy QMessageBox do pokazania rozwiązania jako kod
        msg_box = QMessageBox(self)
        msg_box.setWindowTitle("Rozwiązanie")
        # Formatujemy rozwiązanie jako preformatowany tekst (kod)
        formatted_solution = f"<pre><code>{self.solution.strip()}</code></pre>"
        msg_box.setText(formatted_solution)
...
class LessonScreen(QWidget):
    ...
    def init_ui(self):
        ...
        main_layout.setContentsMargins(20, 20, 20, 20)  # Dodajemy trochę oddechu
        main_layout.setSpacing(15)

        # --- TYTUŁ LEKCJI ---
        self.title_label = QLabel(f"📖 Lekcja {self.lesson_index}: Teoria")
        # Używamy objectName, aby móc stylizować go z pliku .qss
        self.title_label.setObjectName("lessonTitle")
        self.title_label.setAlignment(Qt.AlignCenter)
        main_layout.addWidget(self.title_label)

        # --- POLE TEKSTOWE Z TREŚCIĄ LEKCJI ---
        self.text_edit = QTextEdit()
        self.text_edit.setReadOnly(True)
        # Ustawiamy minimalną wysokość, aby tekst był czytelny
        self.text_edit.setMinimumHeight(300)
        main_layout.addWidget(self.text_edit)

        # --- UKŁAD PRZYCISKÓW NAWIGACYJNYCH (obok siebie) ---
        ...
        # Dodajemy układ nawigacji do głównego układu
        main_layout.addLayout(nav_layout)

        # --- PRZYCISKI AKCJI ---
        self.task_button = QPushButton("✍️ Przejdź do zadań")
        self.task_button.setObjectName("taskButton")  # ID do stylów
        # Ustawiamy większą wysokość przycisków akcji
        self.task_button.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        self.task_button.setMinimumHeight(45)
        self.task_button.clicked.connect(lambda: self.main_window.show_task())
        main_layout.addWidget(self.task_button)

        self.programmer_task_button = QPushButton("💻 Zadanie Programisty (Opcjonalne)")
        self.programmer_task_button.setObjectName("secondaryButton")  # Styl dodatkowy
        self.programmer_task_button.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        self.programmer_task_button.setMinimumHeight(45)
        self.programmer_task_button.clicked.connect(self.show_programmer_task)
        main_layout.addWidget(self.programmer_task_button)

        self.test_button = QPushButton("🚀 Rozpocznij test modułu")
        self.test_button.setObjectName("finalTestButton")  # ID do stylów
        ...
        # --- PRZYCISK POWROTU (na samym dole) ---
        main_layout.addStretch()  # Dodaje pustą, rozciągliwą przestrzeń
    ...
    def update_page(self):
        # Zmieniamy setPlainText na setHtml, aby renderować formatowanie
        self.text_edit.setHtml(self.theory_pages[self.page])
        self.prev_button.setEnabled(self.page > 0)
        self.next_button.setEnabled(self.page < len(self.theory_pages) - 1)

        # Pokaż przycisk do zadań/testu tylko na ostatniej stronie
    ...
    def show_programmer_task(self):
        """Wczytuje dane i pokazuje okno z Zadaniem Programisty."""
        print(f"DEBUG: Próba otwarcia Zadania Programisty dla modułu {self.lesson_index}")
        task_data = load_programmer_task_data(self.lesson_index)
        if not task_data["solution"]:  # Proste sprawdzenie, czy zadanie istnieje
            QMessageBox.information(self, "Informacja", "Brak dodatkowego zadania programisty dla tego modułu.")
            return
    ...
    def get_theory_pages(self, lesson_index):
        # Używamy prostego HTML do formatowania tekstu
        # <pre><code>...</code></pre> to idealny tag do pokazywania kodu
        # <h2> to nagłówek, <b> to pogrubienie, <ul><li> to lista
        if lesson_index == 1:
            return [
        # Strona 1: Co to są zmienne (bez zmian)
        """
        <h2>Co to są zmienne? 📦</h2>
        Wyobraź sobie, że zmienna to <b>pudełko z etykietą</b>...
        <pre><code>wiek = 25\nimie = "Ala"</code></pre>
        """,
        # Strona 2: Typy danych (bez zmian)
        """
        <h2>Typy danych</h2>
        Python automatycznie rozpoznaje, co wkładasz do "pudełka"...
        <ul><li><b>int</b>...</li><li><b>float</b>...</li><li><b>str</b>...</li><li><b>bool</b>...</li></ul>
        <pre><code>liczba_uczniow = 20\nsrednia_ocen = 4.5...</code></pre>
        """,
        # Strona 3: Nazwy zmiennych (bez zmian)
        """
        <h2>Dobre i złe nazwy zmiennych ✅❌</h2>
        Nazwy zmiennych muszą przestrzegać kilku zasad...
        <pre><code># ✅ Poprawne nazwy...\n# ❌ Niepoprawne nazwy...</code></pre>
        """,
        # Strona 4: Operacje na zmiennych (bez zmian)
        """
        <h2>Operacje na zmiennych ⚙️</h2>
        Gdy masz już dane w zmiennych, możesz wykonywać na nich różne operacje...
        <pre><code>a = 10\nb = 5\nsuma = a + b...</code></pre>
        """,
        # --- NOWA STRONA 5: Interakcja z użytkownikiem ---
        """
...
print("Twoja liczba to: " + tekst_z_liczby)
        ...
        <b>Uwaga:</b> Próba konwersji tekstu, który nie wygląda jak liczba (np. `int("cześć")`) spowoduje błąd `ValueError`.
        <br><br>
        Gratulacje! Znasz już podstawy zmiennych i interakcji. Czas na zadania!
        """
...
print(f"Suma liczb: {suma}") # Wypisze 60</code></pre>
            ...
            return ["<h1>Brak teorii dla tego modułu</h1>"]