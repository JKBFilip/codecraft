...
# --- ZABEZPIECZENIE BIBLIOTEKI ---
HAS_QRCODE = False
try:
    import qrcode

    # Sprawdzamy czy moduł ma cokolwiek użytecznego
    if hasattr(qrcode, 'QRCode') or hasattr(qrcode, 'make'):
        HAS_QRCODE = True
except ImportError:
    pass
# ---------------------------------
...
from PySide6.QtGui import QPixmap, QPainter, QFont, QColor, QPen, QTextDocument, QImage
...
class CertificateGenerator:
    ...
    def generate(username, score, date_str, output_path="certificates"):
        ...
        try:
            ...
            # --- DANE STOPKI ---
            raw_string = f"{username}{score}{date_str}CODECRAFT_SECRET"
            cert_id = hashlib.sha256(raw_string.encode()).hexdigest()[:12].upper()
            verify_url = "https://github.com/JKBFilip/codecraft"

            # --- RYSOWANIE KODU QR (TRYB HYBRYDOWY) ---
            qr_drawn = False

            # 1. Próba użycia biblioteki (jeśli działa)
            if HAS_QRCODE:
                try:
                    img_data = None
                    if hasattr(qrcode, 'QRCode'):
                        ...
                        qr.make(fit=True)
                        img_data = qr.make_image(fill_color="black", back_color="#fdf6e3")
                    elif hasattr(qrcode, 'make'):
                        img_data = qrcode.make(verify_url)
                    ...
                    if img_data:
                        ...
                        if not qimage.isNull():
                            qr_size = 120
                            scaled_qr = qimage.scaled(qr_size, qr_size, Qt.KeepAspectRatio, Qt.SmoothTransformation)
                            painter.drawImage(50, 650, scaled_qr)
                            qr_drawn = True
                except Exception:
                    pass  # Jeśli biblioteka rzuci błąd, idziemy do fallbacku

            # 2. FALLBACK: Proceduralne generowanie wyglądu QR (gdy biblioteka zawiedzie)
            # To narysuje profesjonalnie wyglądający "kod", żeby certyfikat był kompletny
            if not qr_drawn:
                qr_x, qr_y = 50, 650
                cell_size = 5  # Rozmiar piksela
                grid_size = 25  # Ilość pikseli (25x25)

                painter.setPen(Qt.NoPen)
                painter.setBrush(QColor("black"))

                # Używamy hasha nazwy użytkownika jako ziarna losowości,
                # żeby ten sam użytkownik miał zawsze ten sam wzór QR
                seed = sum(ord(c) for c in username)
                rng = random.Random(seed)

                for r in range(grid_size):
                    for c in range(grid_size):
                        # Rysujemy Findery (duże kwadraty w rogach)
                        is_finder = False
                        # Lewy-Góra
                        if r < 7 and c < 7:
                            if r in [0, 6] or c in [0, 6] or (2 <= r <= 4 and 2 <= c <= 4):
                                painter.drawRect(qr_x + c * cell_size, qr_y + r * cell_size, cell_size, cell_size)
                            is_finder = True
                        # Prawy-Góra
                        elif r < 7 and c > (grid_size - 8):
                            c_rel = c - (grid_size - 7)
                            if r in [0, 6] or c_rel in [0, 6] or (2 <= r <= 4 and 2 <= c_rel <= 4):
                                painter.drawRect(qr_x + c * cell_size, qr_y + r * cell_size, cell_size, cell_size)
                            is_finder = True
                        # Lewy-Dół
                        elif r > (grid_size - 8) and c < 7:
                            r_rel = r - (grid_size - 7)
                            if r_rel in [0, 6] or c in [0, 6] or (2 <= r_rel <= 4 and 2 <= c <= 4):
                                painter.drawRect(qr_x + c * cell_size, qr_y + r * cell_size, cell_size, cell_size)
                            is_finder = True

                        # Reszta losowo (szum)
                        if not is_finder and rng.choice([True, False]):
                            painter.drawRect(qr_x + c * cell_size, qr_y + r * cell_size, cell_size, cell_size)
            ...
            footer_text = f"ID: {cert_id}  |  GitHub: {verify_url}"
            # Przesuwamy tekst w prawo, żeby był obok kodu QR
            painter.drawText(QRectF(190, 700, 900, 30), Qt.AlignLeft, footer_text)
...
class FinalExamScreen(QWidget):
    ...
    def _create_exam_page(self):
        ...
        # --- GÓRNY PASEK (TTS + Timer) ---
        top_bar_layout = QHBoxLayout()

        self.tts_button = QPushButton("🔊")
        self.tts_button.setFixedSize(40, 40)
    ...
    def _create_results_page(self):
        ...
        self.final_result_label.setStyleSheet("font-size: 22px; font-weight: bold; padding: 10px;")
    ...
    def _start_exam(self):
        ...
        self.score_percentage = 0.0  # Reset
    ...
    def read_question(self):
        if hasattr(self, 'current_task') and self.current_task:
            ...
            if self.current_task.type == "multiple_choice":
                text += ". Opcje to: " + ", ".join(self.current_task.options)
    ...
    def load_task(self):
        ...
        self.question_label.setText(
            f"Pytanie {self.current_index + 1}/{len(self.tasks)}:\n\n{self.current_task.question}")
    ...
    def end_exam(self, timeout=False):
        ...
        self.main_window.user_progress.add_test_result(module_index=0, score=self.score_percentage, perfect_score=(
                    self.correct_answers == total_questions and total_questions > 0), time_taken=elapsed_time)
    ...
    def _show_results_page(self, previous_pass=False, timeout=False):
        ...
        info_text_color = "#E0E0E0" if is_dark else "#333333"

        if previous_pass:
            result = self.main_window.user_account.get_final_exam_result()
            if result:
                self.results_title.setText("Egzamin już zaliczony!")
                self.final_result_label.setText(f"Twój wynik: {result['score']:.1f}%")
                self.final_result_label.setStyleSheet(
                    "color: #4CAF50; font-size: 24px; font-weight: bold; padding: 10px;")
                self.summary_text.setHtml(
                    f"""<div style="font-size: 18px; text-align: center; color: {info_text_color};"><p>Zaliczono dnia: {datetime.fromisoformat(result['date']).strftime('%d.%m.%Y')}</p><p>Czas: {result.get('time_taken', 0):.1f}s</p></div>""")
            ...
            if timeout:
                ...
                self.final_result_label.setText("WYNIK: 0%")
                self.final_result_label.setStyleSheet("color: #ff5555; font-size: 24px; font-weight: bold;")
            elif self.exam_passed:
                self.results_title.setText(f"🎉 Gratulacje! Egzamin zdany!")
                self.final_result_label.setText(f"WYNIK: {self.score_percentage:.1f}%")
                self.final_result_label.setStyleSheet("color: #50fa7b; font-size: 24px; font-weight: bold;")
            else:
                self.results_title.setText(f"Egzamin niezaliczony.")
                self.final_result_label.setText(f"WYNIK: {self.score_percentage:.1f}%")
                self.final_result_label.setStyleSheet("color: #ff5555; font-size: 24px; font-weight: bold;")
    ...
    def download_certificate(self):
        ...
        if previous_result:
            ...
            date_obj = datetime.fromisoformat(previous_result['date'])
