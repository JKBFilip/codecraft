...
class Achievement:
    ...
    icon: str
    points: int
    condition: Callable[['UserAccount'], bool]
    hidden: bool = False
    ...
    def _initialize_achievements(self) -> List[Achievement]:
        return [
            ...
            Achievement("ultimate_perfectionist", "Ultra Perfekcjonista", "Zalicz wszystkie testy modułów na 100%.",
                        "🏆", 100,
                        lambda u: all(any(t.get('module') == mod and t.get('score', 0) == 100 for t in u.test_history) for mod in range(1, 6)), hidden=True),
            Achievement("speedrunner_3min", "Speedrunner (amator)", "Ukończ jakikolwiek test w czasie poniżej 3 minut.",
                        "⏱️", 10,
                        lambda u: any(t.get('time_taken', 0) > 0 and t.get('time_taken', 999) < 180 for t in u.test_history), hidden=True),
            Achievement("speedrunner_2min", "Speedrunner (pro)", "Ukończ jakikolwiek test w czasie poniżej 2 minut.",
                        "⚡", 25,
                        lambda u: any(t.get('time_taken', 0) > 0 and t.get('time_taken', 999) < 120 for t in u.test_history), hidden=True),
            ...
            Achievement("final_exam", "Mistrz Programowania", "Zdałeś egzamin końcowy z wynikiem co najmniej 80%.", "🏅", 150,
                        lambda u: any(t.get('module') == 0 and float(t.get('score', 0)) >= 80.0 for t in u.test_history))
        ]
